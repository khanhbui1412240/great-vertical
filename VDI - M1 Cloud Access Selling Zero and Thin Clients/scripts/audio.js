...
//     });
    ...
    this.volumeControl = _V_.createElement("div", {
      className: "vjs-volume-control-r",
      innerHTML: "<div class='image-volume-control'></div><div class='dashboard-volume volume-visibility'><div class='volume-slide-bar'><span class='volume-value'></span></div></div>"
    });
    ...
    this.playProgress = _V_.createElement("div", { className: "image-scrubber" });
  ...
  handleDashboarVolume: function(){
    var _this = this;
    var volumeDefault = 80;
    var positionMax = $('.volume-slide-bar').offset().top + 100;
    var postionDrag;
    var holder = false;
    $('.image-volume-control , .image-volume-mute-control').click(function(){
      if ($('.dashboard-volume').hasClass('volume-visibility')) {
        $('.dashboard-volume').removeClass('volume-visibility');
      }else {
        $('.dashboard-volume').addClass('volume-visibility');
      }
    })
    $('.volume-slide-bar').mouseup(function(event){
      
      var volume = positionMax - event.clientY;
      var volumePosition = positionMax - event.clientY;
      
      if (volumePosition < 10) volumePosition = 10;
      if (volumePosition > 100) volumePosition = 100;
      if (volume < 10) volume = 0;
      if (volume > 100) volume = 100;
            
      $('.volume-value').css('height', volumePosition+'px');
      _this.audio.volume = parseFloat(volume / 100);
      holder = false;
      if (volume == 0) {
        $('.image-volume-control').addClass('image-volume-mute-control')
      } else {
        $('.image-volume-control').removeClass('image-volume-mute-control')
      }
      event.preventDefault();
    });
    $('.dashboard-volume').mousemove(function(event){
      event.stopPropagation();
      postionDrag = positionMax - event.clientY;
      volume = positionMax - event.clientY;
      if (postionDrag < 10) postionDrag = 10;
      if (postionDrag > 100) postionDrag = 100;
      if (volume < 0) volume = 0;
      if (volume > 100) volume = 100;
      if (holder == true ) {
        $('.volume-value').css('height', postionDrag+'px');
        _this.audio.volume = parseFloat(volume / 100);
        if (volume == 0) {
          $('.image-volume-control').addClass('image-volume-mute-control')
        } else {
    ...
    $('.volume-slide-bar').mousedown(function(event){
      event.stopPropagation();
      holder = true
      event.preventDefault();
    });
    $('.dashboard-volume').mouseleave(function(event){
      event.stopPropagation();
      holder = false;
      event.preventDefault();
    });
